...
function orderHomeSections(items) { return [...items].sort((a, b) => { const aBanner = a.type === 'BANNER'; const bBanner = b.type === 'BANNER'; if (aBanner !== bBanner) return aBanner ? -1 : 1; return (Number(a.sort) || 0) - (Number(b.sort) || 0) }) }
...
export const usePageContentStore = defineStore('page-content', () => {
  ...
  const homeSections = computed(() => orderHomeSections((home.value?.sections || []).filter((item) => item.enabled)))
