<template>
  <view class="order-detail-page">
    ...
    <view class="detail-order-card"><view class="detail-order-card__head"><text>黑盘羊沉香堂</text><text>{{ order.items }} 件商品</text></view><view class="detail-order-product"><image src="https://images.unsplash.com/photo-1602523961358-f9f03dd557db?auto=format&fit=crop&w=400&q=80" mode="aspectFill" /><view><text>天然沉香收藏香品</text><text>标准收藏装 × {{ order.items }}</text><text>¥{{ order.amount.toFixed(2) }}</text></view></view><view class="detail-order-total">商品总价 <text>¥{{ order.amount.toFixed(2) }}</text></view></view>
